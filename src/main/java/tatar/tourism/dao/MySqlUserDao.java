...
public class MySqlUserDao extends MySqlDao implements UserDao {
    ...
    public void update(User user) {
        ...
        Connection con = getConnection();
        try {
            stmt = con.prepareStatement("UPDATE users SET username =  ?, email=?, role=?, firstname=?, lastname=?, locale=?, active=?, confirmed=? " +
                    "WHERE id =  ?");
            stmt.setString(1, user.getUsername());
    ...
    @Override
    public User read(String username, String password) throws NoSuchAlgorithmException {
